 ...
 * dependencias externas (usa apenas modulos nativos do Node.js).
 *
 ...
 * - Zero dependencias de runtime: sobe com "node mock-app/server.js", sem
 *   precisar de "npm install" previamente.
 *
 * A aplicacao expõe:
...
function matchRoute(method, pathname) {
  for (const route of routes) {
    ...
    const paramNames = [];
    const regexStr = route.pattern.replace(/:([A-Za-z0-9_]+)/g, (_, name) => {
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    await matched.handler(req, res, { params: matched.params, query: url.searchParams, session });
  } catch (err) {
